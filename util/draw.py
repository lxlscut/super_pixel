...
class Draw:
    ...
    def draw(self, src, src_point):
        ...
        for i in range(src_point.shape[0]):
            cv2.circle(result, (int(src_point[i, 0]), int(src_point[i, 1])), 5, color=(b, g, r), thickness=2)
        return result
    ...
    def draw_contours(self, src, src_point):
        """
        :param img: 图片
        :param points: 图片中需要上色的点
        :return: 返回已绘制点的图片
        ...
        r = random.randint(0, 255)
        for i in range(src_point.shape[0]):
            cv2.circle(result, (int(src_point[i, 0]), int(src_point[i, 1])), 1, color=(b, g, r), thickness=1)
        return result